...
final class DataTableWireResources
{
    ...
    {
        if ($this->brokenConfig) {
            throw new TypeError('Internal config failure');
        }
    ...
    public function getResource(string $name): object
    {
        ...
        if ($name === 'doctrine2') {
            $this->doctrineReads++;
            if ($this->brokenRepositoryLogic) {
                throw new LogicException('Internal repository logic failure');
            }
...
foreach (['Domain', 'Mailbox', 'Alias', 'Archive', 'Log'] as $name) {
    $boundary = new ReflectionMethod(
        'ViMbAdmin\\Kernel\\Controller\\' . $name . 'Controller',
    ...
    $parse = static function (string $encoded) use ($boundary, $name): DataTableQuery {
        ...
        if (!is_array($parameters)) {
            throw new RuntimeException('Controller boundary must return an array');
        }
        $typedParameters = [];
        foreach ($parameters as $key => $value) {
            if (!is_string($key)) {
                throw new RuntimeException('Controller boundary must return string keys');
            }
    ...
    $check(
        $name . ': real URL encoding crosses the controller boundary',
    ...
    try {
        ...
        foreach ($malformedRequests as $shape => $request) {
            ...
            $check(
                $name . ': malformed nested ' . $shape . ' returns 400 before repository access',
        ...
        if ($name === 'Mailbox') {
            ...
            $response = $action->invoke($controller);
            $check(
                $name . ': configuration LogicException preserves the 200 ko response',
        ...
        $check(
            $name . ': valid request reaches repository and preserves internal TypeError',
            $repositoryFailed
        );
    } finally {
        $_GET = $oldGet;
