...
$check(
    'queue trigger retries short socket writes until the request is complete',
    is_resource($stream)
...
$check(
    'queue trigger fails closed when a socket write makes no progress',
...
$check(
    'canonical positive integer accepts decimal strings and native ints',
    $invoke('positiveIntegerOrNull', '7') === 7 && $invoke('positiveIntegerOrNull', 9) === 9
);
$check(
    'integer-coercion mutant is killed by malformed and non-positive ids',
...
$check(
    'absent integer configuration alone receives its shipped default',
...
$check(
    'quota multiplier allowlist preserves configured units',
...
$check(
    'string validator rejects arrays before downstream field rules',
    $stringRule(['not-a-string']) === 'This field has an invalid type.' && $stringRule('value') === null
);
$check(
    'optional email storage preserves null rather than inventing an empty string',
...
$check(
    'queue key is optional but present malformed configuration fails before mutation',
...
$check(
    'queue endpoint parser accepts safe server scalars and rejects hostile runtime values',
...
$check(
    'invalid email form containers have safe non-echoing render values',
    $invoke('renderStringOrDefault', ['username'], 'username') === 'username'
        && $invoke('renderStringOrDefault', ['attacker@example.test'], '') === ''
);
$check(
    'typed settings rebuild preserves supported scalar display values and rejects arrays',
...
$check(
    'server port accepts a valid scalar and rejects coercible junk',
...
$check(
    'email callable contract returns only null or a string',
