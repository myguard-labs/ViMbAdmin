...
$check(
    'queue positive ids reject coercion and preserve canonical decimals',
...
$check(
    'queue runner defaults are retained but malformed values fail closed',
...
$check(
    'trusted proxy lists retain numeric entries without array casts',
...
$check(
    'archive list ids reject mixed numeric strings before repository access',
...
$check(
    'archive snapshots preserve exact mailbox fields and scalar types',
    $snapshot === [
        'username' => 'alice@example.test', 'local_part' => 'alice', 'name' => null,
        'password' => '{SSHA}hash', 'quota' => 1024, 'active' => true,
    ]
...
$check(
    'maintenance confirmation accepts only the shipped binary forms',
...
$check(
    'maintenance maildir names cannot escape the configured root',
...
$check(
    'malformed orphan confirmation fails before filesystem or entity-manager access',
...
$check(
    'malformed orphan username fails before the orphan scan',
