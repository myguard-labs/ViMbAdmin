...
function check(string $label, bool $ok): void
{
    ...
    if (!$ok) {
        TestKernelSessionNamespaceHarnessState::$count++;
    }
}
...
check(
    'set writes through to $_SESSION slot',
...
check(
    'unset removes the $_SESSION key',
...
check(
    'Application namespace unaffected by ViMbAdmin_Auth write',
    $app->__get('flashMessages') === ['hi']
);
check(
    'ViMbAdmin_Auth namespace stored separately',
