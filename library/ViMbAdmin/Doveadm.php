...
class ViMbAdmin_Doveadm
{
    ...
    /**
     * Recursive on-disk byte size of a backup directory, via the doveadm `fs`
     * commands (no shared filesystem with ViMbAdmin). `fs iter` is NOT
     * recursive, so we walk: iter a dir, `fs stat` each entry; an entry that
     * `fs stat` reports with size 0 AND iterates further is treated as a
     * subdirectory and recursed into. The returned bytes are the ACTUAL
     * on-disk size (the maildir is zstd-compressed by Dovecot's mail_compress,
     * so this is the compressed footprint, not the logical mail size).
     *
     * Best-effort + bounded: called ONCE at archive time (not per page render),
     * with a depth cap so a pathological tree can't loop. Any doveadm error
     * yields null (the caller stores null and the UI shows "—").
     *
     * @param string $path    filesystem path or `<driver>:<path>` dest URI
     * @param string $filter  the fs filter name (default "posix")
    ...
    {
        if( $depth > 12 )           // maildir nesting is shallow; cap to be safe
            return 0;

        $entries = $this->run( 'fsIter', [
            'filterName' => $filter,
            'path'       => [ $dir . '/' ],
        ] );
        if( !is_array( $entries ) )
        ...
        {
            $name = is_array( $e ) ? ( $e['path'] ?? null ) : $e;
            if( $name === null || $name === '' || $name === '.' || $name === '..' )
                continue;
            $child = $dir . '/' . ltrim( (string) $name, '/' );
            ...
            // size 0 / unstattable -> treat as a directory and recurse.
