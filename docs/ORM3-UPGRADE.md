...
`src/Kernel/Doctrine/EntityManagerFactory.php`. It uses attribute mappings through
Doctrine's `AttributeDriver`, a DBAL connection from `DriverManager`, and direct
PSR-6 pools from `symfony/cache` for metadata, query, and result caches. A custom
...
and runs schema-drift checks against the attribute mappings. PHPStan is enforced at
