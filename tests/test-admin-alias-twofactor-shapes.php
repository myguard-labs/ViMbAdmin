...
$check(
    'admin IDs preserve only positive canonical integers',
...
$check(
    'admin form strings and booleans reject container truthiness',
...
$check(
    'malformed POST scalar fields become inert rather than stringifying',
    $invoke(AdminController::class, 'postStringOrEmpty', ['enable']) === ''
        && $invoke(AdminController::class, 'postStringOrEmpty', 'enable') === 'enable'
);
$check(
    'malformed enrolment session state is regenerated by its caller',
...
$check(
    'admin authentication options preserve an exact string-keyed map',
    $invoke(AdminController::class, 'requiredOptionArray', $auth, 'resources', 'auth', 'oss')
        === ['pwhash' => 'crypt:sha512']
);
$check(
    'admin security configuration fails closed on missing, scalar and list shapes',
...
$check(
    'security salt defaults only when absent and rejects containers',
...
$check(
    'alias IDs reject direct-cast ambiguity',
...
$check(
    'alias binary flags preserve documented encodings',
...
$check(
    'alias DataTables scalar boundary never accepts container values',
...
$check(
    'alias DataTables boundary retains modern nested fields',
    $invoke(AliasController::class, 'requestArray', $modernRequest) === $modernRequest
);
$check(
    'alias DataTables boundary still rejects unrelated containers',
    $fails(static fn (): mixed => $invoke(AliasController::class, 'requestArray', ['did' => ['1']]))
);
$check(
    'alias pagination config distinguishes absent from malformed',
...
$check(
    'orphan alias relation fails before authorization or plugin context',
...
$check(
    'TOTP normalizer accepts exact codes and rejects coercive shapes',
...
$check(
    'TOTP verification rejects malformed secret and code containers',
    (new ReflectionMethod($twoFactor, 'verifyCode'))->invoke($twoFactor, ['secret'], ['123456']) === false
);
$check(
    'replay timestamp parser accepts canonical non-negative values only',
...
$check(
    'missing replay state starts at zero while corrupt state fails closed',
...
$check(
    '2FA enabled state requires a non-empty stored string',
...
$check(
    'object-shaped backup-code state fails closed',
    $twoFactor->consumeBackupCode($preferenceAdmin, '23456789AB') === false
        && $twoFactor->backupCodesRemaining($preferenceAdmin) === 0
);
$check(
    'backup-code input containers cannot stringify into credentials',
...
$check(
    'valid backup codes remain single-use and update the exact remaining count',
