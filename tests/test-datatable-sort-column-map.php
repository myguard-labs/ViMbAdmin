...
$check(
    'domain map with the size column rendered follows the rendered order',
...
// 'domain' for Created.
$check(
    'domain map with the size column hidden shifts every later column down one',
...
$check(
    'domain Created sorts by created under the shipped default, not by domain',
    $sortField(DomainController::class, 7, false) === 'created'
);
$check(
    'domain Transport sorts by transport under the shipped default, not by active',
...
$check(
    'log map without a domain filter follows the rendered order',
...
$check(
    'log map with a domain filter drops the Domain column',
    $mapOf(LogController::class, 3, true) === ['action', 'timestamp', 'admin', 'timestamp']
);
$check(
    'domain-scoped log index 3 is the Occurred At column, not domain',
...
$check(
    'mailbox map with the domain column rendered follows the rendered order',
...
$check(
    'mailbox map without the domain column exposes no domain sort',
