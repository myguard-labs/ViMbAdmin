...
$check(
    'canonical positive ids preserve integer identity',
    $invoke('positiveIntegerOrNull', 7) === 7
        && $invoke('positiveIntegerOrNull', '12') === 12
);
$check(
    'direct-cast mutant is killed by ambiguous route ids',
...
$check(
    'DataTables boundary retains modern scalar and nested parameters',
...
$check(
    'absent and canonical INI booleans preserve pagination semantics',
...
foreach ([
    ...
    $check(
        "domain list-data enforces {$case} search minimum argument",
        $response->status === 400
...
$check(
    'quota multiplier accepts the complete shipped allowlist case-insensitively',
...
$check(
    'checkbox values preserve native and HTML form booleans',
...
$check(
    'non-negative integer parsing rejects lossy coercion',
...
$check(
    'quota conversion preserves validated finite byte values',
...
$check(
    'default rendering accepts documented non-negative INI numbers',
    $invoke('nonNegativeIntegerDefault', 12, 'Aliases') === '12'
        && $invoke('nonNegativeIntegerDefault', '12', 'Aliases') === '12'
        && $invoke('nonNegativeNumberDefault', 1.5, 'Quota') === '1.5'
);
$check(
    'default rendering rejects containers, booleans and negative values',
...
$check(
    'explicit null domain defaults cannot masquerade as absent',
...
$check(
    'invalid present add alias id fails closed before repository access',
...
$check(
    'administrator removal rejects missing CSRF before lookup or mutation',
...
$check(
    'active toggle rejects missing CSRF before lookup or mutation',
...
$check(
    'administrator removal posts the CSRF token in the request body (VIM-D05)',
...
$check(
    'active-toggle request carries the rendered CSRF token',
...
$check(
    'validated form mapping preserves exact entity values',
...
$check(
    'late malformed form value causes zero partial entity mutation',
    $lateFailure
